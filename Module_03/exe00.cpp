...
{
	std::cout << GREEN "[ClapTrap] - " < < < < "Constructor called with name" << RESET << std::endl;
}
...
{
	std::cout << YELLOW "[ClapTrap] - " < < < < "Assignation operator called" << RESET << std::endl;
	if (this != &tmp)
	{
		_name = tmp._name;
		_hit = tmp._hit;
...
ClapTrap::~ClapTrap() { std::cout << RED "[ClapTrap] - " < < < < "Destructor called" << RESET << std::endl; }
